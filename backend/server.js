...
app.post('/upload-avatar/:id', upload.single('avatar'), async (req, res) => {
  ...
  // Use PUBLIC_BASE_URL env var in development to ensure clients can reach the returned URL.
  // Fallback to the request host if PUBLIC_BASE_URL is not provided.
  // Example: PUBLIC_BASE_URL=http://192.168.1.100:3000 npm start
  const PUBLIC_BASE = process.env.PUBLIC_BASE_URL || `${req.protocol}://${req.get('host')}`;
  const avatarUrl = `${PUBLIC_BASE}/uploads/${req.file.filename}`;
